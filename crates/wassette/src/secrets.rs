...
impl SecretsManager {
    ...
    pub async fn ensure_secrets_dir(&self) -> Result<()> {
        if !self.secrets_dir.exists() {
            ...
            tokio::fs::create_dir_all(&self.secrets_dir).await
                .with_context(|| format!("Failed to create secrets directory: {}", self.secrets_dir.display()))?;
        }
        ...
        #[cfg(unix)]
        {
            let metadata = tokio::fs::metadata(&self.secrets_dir).await
                .with_context(|| format!("Failed to get metadata for secrets directory: {}", self.secrets_dir.display()))?;
            let mut perms = metadata.permissions();
            perms.set_mode(0o700);
            tokio::fs::set_permissions(&self.secrets_dir, perms).await
                .with_context(|| format!("Failed to set permissions for secrets directory: {}", self.secrets_dir.display()))?;
        }
    ...
    pub async fn load_component_secrets(&self, component_id: &str) -> Result<HashMap<String, String>> {
        ...
        let metadata = tokio::fs::metadata(&secrets_path).await
            .with_context(|| format!("Failed to get metadata for secrets file: {}", secrets_path.display()))?;
        let mtime = metadata.modified()
            .with_context(|| format!("Failed to get modification time for secrets file: {}", secrets_path.display()))?;
        ...
        let content = tokio::fs::read_to_string(&secrets_path).await
    ...
    pub async fn list_component_secrets(&self, component_id: &str, show_values: bool) -> Result<HashMap<String, Option<String>>> {
    ...
    pub async fn set_component_secrets(&self, component_id: &str, secrets: &[(String, String)]) -> Result<()> {
        ...
        let mut existing_secrets = if secrets_path.exists() {
            let content = tokio::fs::read_to_string(&secrets_path).await
                .with_context(|| format!("Failed to read existing secrets file: {}", secrets_path.display()))?;
            serde_yaml::from_str::<HashMap<String, String>>(&content)
                .with_context(|| format!("Failed to parse existing secrets file: {}", secrets_path.display()))?
        } else {
            HashMap::new()
        };
        ...
        // Write atomically
        self.write_secrets_file(&secrets_path, &existing_secrets).await?;
    ...
    pub async fn delete_component_secrets(&self, component_id: &str, keys: &[String]) -> Result<()> {
        ...
        if !secrets_path.exists() {
            return Err(anyhow!("No secrets file found for component: {}", component_id));
        }
        ...
        for key in keys {
            if secrets.remove(key).is_none() {
                warn!("Secret key '{}' not found for component: {}", key, component_id);
            }
        }
        ...
        if secrets.is_empty() {
            ...
            tokio::fs::remove_file(&secrets_path).await
                .with_context(|| format!("Failed to remove empty secrets file: {}", secrets_path.display()))?;
            ...
            info!("Deleted {} secret(s) for component: {}", keys.len(), component_id);
        }
    ...
    async fn write_secrets_file(&self, secrets_path: &Path, secrets: &HashMap<String, String>) -> Result<()> {
        ...
        tokio::fs::write(&temp_path, &content).await
            .with_context(|| format!("Failed to write temporary secrets file: {}", temp_path.display()))?;

        // Set file permissions to 0600 (user read/write only)
        #[cfg(unix)]
        {
            let metadata = tokio::fs::metadata(&temp_path).await
                .with_context(|| format!("Failed to get metadata for temporary secrets file: {}", temp_path.display()))?;
            let mut perms = metadata.permissions();
            perms.set_mode(0o600);
            tokio::fs::set_permissions(&temp_path, perms).await
                .with_context(|| format!("Failed to set permissions for temporary secrets file: {}", temp_path.display()))?;
        }

        // Atomic rename
        tokio::fs::rename(&temp_path, secrets_path).await
            .with_context(|| format!("Failed to rename temporary secrets file to: {}", secrets_path.display()))?;
...
mod tests {
    ...
    fn test_sanitize_component_id() {
        ...
        assert_eq!(sanitize_component_id("with_underscores"), "with_underscores");
    ...
    async fn test_secrets_manager_basic() -> Result<()> {
        ...
        manager.set_component_secrets("test-component", &secrets).await?;
        ...
        let listed = manager.list_component_secrets("test-component", false).await?;
        assert_eq!(listed.len(), 2);
        assert!(listed.contains_key("API_KEY"));
        assert!(listed.contains_key("REGION"));
        assert_eq!(listed.get("API_KEY"), Some(&None));

        let listed_with_values = manager.list_component_secrets("test-component", true).await?;
        assert_eq!(listed_with_values.get("API_KEY"), Some(&Some("secret123".to_string())));

        // Test deleting secrets
        manager.delete_component_secrets("test-component", &["API_KEY".to_string()]).await?;
